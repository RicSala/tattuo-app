    ...
    callbacks: {
        ...
        async jwt({ token }) {
            ...
            if (dbUser.role === "ARTIST") {
                const artistProfile = await ArtistService.getArtistByUserId(
                    dbUser.id,
                );
                token.artistProfileId = artistProfile.id;
                token.artist = artistProfile;

                // # of tattoos
                const tattoos = await TattooService.getByArtistId(
                    artistProfile.id,
                );
                token.artist.numOfTattoos = tattoos.length;
            }

            const favoriteTattooIds =
                await UserService.getFavoriteTattooIds(dbUser);
            const favoriteArtistIds =
                await UserService.getFavoriteArtistIds(dbUser);
            const savedArtists = await UserService.getSavedArtistsByUserId(
                dbUser.id,
            );
            const savedTattoos = await UserService.getSavedTattoosByUserId(
                dbUser.id,
            );
        ...
        async session({ session, token, user }) {
            ...
            if (session && session.user) {
                ...
                if (token.artistProfileId) {
                    ...
                    session.user.artist = token.artist;
                    session.user.artist.numOfTattoos =
                        token.artist.numOfTattoos;
                }
            }
            return session;
        },
